...
#include "img_hash/img_hash_opencv_module/block_mean_hash.hpp"

using namespace std;


namespace hashes{
class BlockMeanHash : public FragmentHash<vector<bool>>
{
private:
    vector<bool> hash;
...
public:
    ...
    BlockMeanHash(ShapeAndPositionInvariantImage frag):
            FragmentHash<vector<bool>>(frag)
    {
        hash_ = computeHash(frag.getImageData());
    }

    BlockMeanHash(string getHashFromString, std::vector<Keypoint> shape=vector<Keypoint>()):
            FragmentHash<vector<bool>>(getHashFromString, shape)
    {
        hash_ = hex_str_to_hash(getHashFromString);
    }

    BlockMeanHash(const BlockMeanHash& that) :
            FragmentHash(that)
    {}
